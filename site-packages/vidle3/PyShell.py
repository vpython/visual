...
class ModifiedInterpreter(InteractiveInterpreter):
    ...
    def showtraceback(self, temp_filename=None):
        ...
        list(map(sys.stderr.write, lines))
...
##        self.config(usetabs=1, indentwidth=8, context_use_ps1=1)
        ...
        text.bind("<<paste>>", self.paste_callback)
    ...
    def paste_callback(self, event):
        ...
        try:
            selection = tk.call('::tk::GetSelection', text, 'CLIPBOARD')
        except TclError:
            # No selection, probably.
            return "break"
        oldseparator = text.cget('autoseparators')
        if oldseparator:
            text.configure(autoseparators=0)
            text.edit_separator()
        if tk.call('tk', 'windowingsystem') != 'x11':
            try:
                text.delete('sel.first', 'sel.last')
            except TclError:
                # Nothing tagged with 'sel', probably.
                pass
